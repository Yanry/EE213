...
class RomGenerator:
    inv_netlist = ".subckt xrom_input_inv a z s=1\n" + \
                  "m1 z a vdd! vdd! PMOS_VTL L=50e-9 W='s*180e-9'\n" + \
                  "m0 z a 0 0 NMOS_VTL L=50e-9 W='s*90e-9'\n" + \
                  ".ends\n"

    and_netlist = ".subckt xrom_input_and a b out s=1\n" + \
    ...
    def __init__(self, in_file_name, out_file_name, col_num, addr_width):
    ...
    def add_inv_inst(self, in_pin, out_pin, size, id):
        ...
        inv_str = "xrom_inv{} {} xrom_input_inv s={}".format(id, pin_str, size)
        self.write_netlist(inv_str)
    
    def add_and_inst(self, a, b, out, size, id):
        pin_str = "{} {} {}".format(a, b, out)
        and_str = "xrom_and{} {} xrom_input_and s={}".format(id, pin_str, size)
    ...
    def gen_and_tree(self):
        ...
        self.write_netlist(".subckt xrom_input_and_tree {}".format(pin_str))
        self.and_count = 0
    ...
    def add_and_tree_inst(self, in_list, out, id):
        ...
        self.write_netlist("xrom_and_tree{} {} xrom_input_and_tree".format(id, pin_str))
    ...
    def generate_decoder(self):
        ...
        pin_str = self.pin_list_to_str(in_pin_list + out_pin_list)
        self.write_netlist(".subckt xrom_input_dec {}".format(pin_str))
    ...
    def generate_array(self):
        ...
        self.write_netlist(".subckt xrom_input_array {}".format(pin_str))
    ...
    def generate_rom(self):
        ...
        out_pin_list = ['input<{}>'.format(i) for i in range(self.col_num)]
        word_line_list = ['wl<{}>'.format(i) for i in range(self.row_num)]

        pin_str = self.pin_list_to_str(in_pin_list + out_pin_list)
        self.write_netlist(".subckt xrom_input {}".format(pin_str))
        ...
        self.write_netlist("xrom_dec_inst {} xrom_input_dec".format(dec_pin_str))
        self.write_netlist("xrom_array_inst {} xrom_input_array".format(array_pin_str))
        self.write_netlist(".ends\n")
...
if __name__ == "__main__":
    assert len(sys.argv) == 5, "Usage: python input_rom_gen.py INPUT_FILE OUTPUT_FILE COL_NUM ADDR_WIDTH"
    infile_name = sys.argv[1]
    outfile_name = sys.argv[2]
    col_num = int(sys.argv[3])
    addr_width = int(sys.argv[4])
    rom_gen = RomGenerator(infile_name, outfile_name,col_num,addr_width)
