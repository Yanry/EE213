...
def check_mac_function(trace,voh,vol):
	...
	for idx in check_idx:
		try:
			multiplier_bits = convert_voltage_to_binary_multibits(get_signal_voltage_multibits(trace,"multiplier",192,idx-2),vih,vil)
			multiplicand_bits = convert_voltage_to_binary_multibits(get_signal_voltage_multibits(trace,"multiplicand",256,idx-2),vih,vil)
			result_bits = convert_voltage_to_binary_multibits(get_signal_voltage_multibits(trace,"result",12,idx-2),vih,vil)
			...
			exit(-1)
		multiplier = [convert_binary_to_signed_integer(x) for x in [multiplier_bits[i:i+3] for i in range(0,192,3)]]
		multiplicand = [convert_binary_to_signed_integer(x) for x in [multiplicand_bits[i:i+4] for i in range(0,256,4)]]
